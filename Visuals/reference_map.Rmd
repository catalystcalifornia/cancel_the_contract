...
map <- ggplot() + 
  ...
  geom_sf(data = spa1, aes(color = "SPA 1 boundary"), linewidth = 0.3, fill = NA) +
  geom_sf(data = my_avuhsd_schools, aes(shape = "AVUHSD school"), size = 1.1) +
  ...
  annotate(geom = "text", x = -118.75, y = 34.68, label = "AVUHSD boundaries\nextend beyond SPA 1\nboundaries south of\nElizabeth Lake", size = 6, hjust = 0, family = font_caption, lineheight = 0.3) +
  
  geom_segment(aes(x = -118.7, y = 34.645, xend = -118.7, yend = 34.625), 
               size = 0.5, arrow = arrow(length = unit(0.1, "cm"))) +
  
  annotate(geom = "text", x = -118.56, y = 34.495, label = "SPA 1 boundaries\nextend beyond\nAVUHSD south\nof Acton", size = 6, hjust = 0, family = font_caption, lineheight = 0.3) +
