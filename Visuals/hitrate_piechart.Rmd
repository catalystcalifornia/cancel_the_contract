...
df_subset <- df %>%
  mutate(label = ifelse(label=="0", "Contraband found", "No contraband found"))

indicator <- "Police Hit Rate AVUHSD"
title_text <- "The majority of LASD deputies' searches of AVUHSD students result in no contraband found"
```
...
pie_chart <- function(df, indicator, title_text) {
  ...
  width <- 5   # inches
  height <- 5  # inches  
  wrap_width <- round(width * 10)

  # Adjust width based on title length
  title_length <- nchar(title_text)
  title_text <- str_wrap(title_text, width = wrap_width)
