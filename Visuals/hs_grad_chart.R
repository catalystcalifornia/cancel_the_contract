...
source("W:/Project/RJS/CTC/Github/CR/cancel_the_contract/Analysis/hs_grad.R")
# View(av_table)
hs_grad <- av_table %>% dplyr::filter(`Adjusted 5-Year 2023-2024`=="Total Rate") %>% 
  dplyr::select(-c(`Adjusted 5-Year 2023-2024`, Male, Female, Migrant, ELL)) %>% 
  mutate_if(is.numeric, round, digits = 2) %>% dplyr::rename("Filipinx"="Filipino")

hs_grad$`Adjusted 5-Year 2023-2024` = "Antelope Valley Union High School District"
...
s_long <- reshape2::melt(s, id.vars=c("Adjusted 5-Year 2023-2024"))
