...
pop_race<-dbGetQuery(con_ctc, "SELECT race, rate FROM av_population_race") %>%
  rename("label" = "race") %>%
  race_recode(.) 
# %>%   filter(label %in% c("Black", "White", "Latinx"))
...
df<- all_stops %>%
  inner_join(pop_race, by = "label")

indicator.y <- "Population"
indicator.x <- "Police Stops by Race"

myindicator.y <- "Antelope Valley Population"
myindicator.x <- "Police Stops"
title_text <- "Police officers stop Black residents at over 2x the rate of their population"
