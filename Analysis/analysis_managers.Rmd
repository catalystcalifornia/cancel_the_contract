...
ppl_sswana_or <- ppl_soasian %>%
  mutate(nh_asian_wo_sa = ifelse(race=="nh_asian" & soasian=="not south asian", "nh_asian_wo_sa", "not nh_asian_wo_sa"),
        sswana = ifelse(swana=="swana" | soasian=="south asian", "sswana", "not sswana")) # QA: I think the statement needs to be an 'and' statement: if a person is swana AND south asian, they should be labeled as SSWANA, otherwise they are NOT sswana. Originally you had an OR statement so that it was: if a person is swana OR south asian, then they are sswana. Using an OR statement produces 335 where someone who is swana but not south asian got recoded as SSWANA. 

ppl_sswana_and <- ppl_soasian %>%
  mutate(nh_asian_wo_sa = ifelse(race=="nh_asian" & soasian=="not south asian", "nh_asian_wo_sa", "not nh_asian_wo_sa"),
        sswana = ifelse(swana=="swana" & soasian=="south asian", "sswana", "not sswana")) # QA: I think the statement needs to be an 'and' statement: if a person is swana AND south asian, they should be labeled as SSWANA, otherwise they are NOT sswana. Originally you had an OR statement so that it was: if a person is swana OR south asian, then they are sswana. Using an OR statement produces 335 where someone who is swana but not south asian got recoded as SSWANA. 

# QA examine the recoding results

sum(is.na(ppl_sswana_and$nh_asian_wo_sa)) # there are 364 NAs
sum(is.na(ppl_sswana_or$nh_asian_wo_sa)) # there are 364 NAs
...
sswana_qa_and<-ppl_sswana_and%>%
  select(race, latino, soasian, nh_asian_wo_sa, swana, sswana)%>%
  filter(soasian=="south asian") # it looks like everyone who is Southasian is also not Swana so they are not labeled as SSWANA which seems right to me. 

sswana_qa_or<-ppl_sswana_or%>%
  select(race, latino, soasian, nh_asian_wo_sa, swana, sswana)%>%
  filter(soasian=="south asian") # it looks like everyone who is Southasian is also not Swana so they are not labeled as SSWANA which seems right to me. 

table(sswana_qa$sswana)
...
# 64.8% closely matches the total rate for Palmdale and Lancaster when looking at the city results on race counts https://www.racecounts.org/city/Palmdale/ and https://www.racecounts.org/city/lancaster/
```
...
# QA
table(ppl$emply) # in labor force: 8526 not in labor force: 4371
...
ppl_calc <- ppl %>%               
  as_survey_rep(
    variables = c(race, aian, latino, swana, pacisl, officials),   # select grouping variables
...
# QA --- use the actual function code to run and see if results match what is produced in av_managers_by_race

  # calculate by latino 
  indicator_latino <- ppl_calc %>%
    group_by(latino, officials) %>%
    summarise(
    ...
    left_join(ppl_calc %>%                                        				  # left join in the denominators 
                group_by(latino) %>%                       
...
variables = c("race", "aian", "latino", "swana", "pacisl")  
