# Antelope Valley Union High School District Stops that were Searches / Hit Rates (stops with a search and no contraband found)
...
av_stops_re<-av_stops%>%
  ...
  mutate(contraband_evidence_discovered_none = case_when(
    contraband_evidence_discovered_none %in% c("Yes", "true")  ~ 1,
    contraband_evidence_discovered_none %in% c("No", "false")  ~ 0,
    TRUE                                    ~ NA_real_
  ))%>%
...
tot<-av_stops_re%>%
  mutate(total=n())%>%
  group_by(search_of_person_conducted)%>%
...
nh<-av_stops_re%>%
  filter(search_of_person_conducted==1)%>%
  mutate(total=n())%>%
  group_by(search_of_person_conducted, reportingcategory_re)%>%
  mutate(count=n(),
         rate=count/total*100)%>%
...
tot_flags <- av_stops_re %>%
  ...
  filter(flag_value == 1 & search_of_person_conducted==1) %>%
...
############### PUSH  SEARCHES TABLE TO POSTGRES #####################
...
table_name <- "analysis_stops_search_avuhsd"
schema <- "data"
indicator <- "Analysis table of AVUSHD LASD stops that containded a search out of all people stopped, as well as stops with a searched disaggregated by 
perceived race,"
...
column_comments <- c(
  "Universe: All searches -meaning out of all people who were searched (n=177) our out of ALL people stopped in AVUHSD (n=904)",
  "geography level",
  "CDE reporting category recoded",
  "Total (see universe for specific total value definition)",
  "Count of LASD stops that contained a search by perceived race in AVUHSD",
  "Percent of LASD stops with a search by perceived race in AVUHSD"
)
