...
svy_data <- raw_svy_data %>% 
  ...
  mutate(fostercare = ifelse(ga==1, 1, 0)) 
...
response_analysis_per_suspended <- dbGetQuery(con_bv, "SELECT * FROM youth_thriving.response_analysis_per_suspended WHERE variable = 'cy'")
response_analysis_per_arrested <- dbGetQuery(con_bv, "SELECT * FROM youth_thriving.response_analysis_per_arrested WHERE variable = 'cy'")

response_analysis_per_suspended_test <- fx_freq_table(svy_data, "suspended") %>% 
  filter(variable == "cy")

response_analysis_per_foster <- fx_freq_table(svy_data, "fostercare") %>% 
  filter(variable == "cy")


```

The code below is ONLY necessary if we want to visualize the data.
